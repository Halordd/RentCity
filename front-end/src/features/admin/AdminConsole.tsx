...
import type { AdminRows, DataRow, NavigateTo, RoutedScreenProps } from "../../types";
...
const defaultAdminData: AdminData = {
  rows: adminService.rows,
  auditRows: adminService.auditRows,
  financeRows: adminService.financeRows,
  metrics: []
};
...
export function AdminConsole({ route, navigate }: RoutedScreenProps) {
  ...
  useEffect(() => {
    ...
    async function loadRemoteAdmin() {
      if (!isApiConfigured() || state.auth?.user.role !== "ADMIN") return;
      try {
        const remote = await adminService.rowsRemote();
        if (!cancelled) setData(remote);
      } catch {
        if (!cancelled) notify("Chưa tải được dữ liệu admin từ backend.");
      }
    }
  ...
  }, [notify, state.auth?.user.role]);

  const pages: Record<string, ReactNode> = {
    overview: <Overview navigate={navigate} data={data} />,
    listings: <Listings navigate={navigate} data={data} />,
    verification: <Verification navigate={navigate} data={data} />,
    automation: <Automation navigate={navigate} data={data} />,
    billing: <Billing navigate={navigate} data={data} />,
    disputes: <Disputes navigate={navigate} data={data} />,
    audit: <Audit navigate={navigate} data={data} />,
    access: <Access navigate={navigate} data={data} />,
    settings: <Settings navigate={navigate} data={data} />
  };
...
function Billing({ navigate, data }: AdminPageProps) { return <><AdminHeader title="Tài chính & đối soát" body="Back-office cho cọc, hoàn tiền, hóa đơn, subscription chủ nhà và giao dịch cần kiểm tra." navigate={navigate} /><TableSection title="Giao dịch cần đối soát" headers={["Mã", "Ngày", "Số tiền", "Trạng thái"]} rows={data.financeRows} /><ActionPanel title="Cổng thanh toán" body="Kiểm tra webhook, biên nhận, mã giao dịch ngân hàng và quyền duyệt hoàn tiền." action="Kiểm tra kết nối" /></>; }
function Disputes({ navigate, data }: AdminPageProps) { return <><AdminHeader title="Trung tâm khiếu nại" body="Xử lý hoàn cọc, ảnh sai, chủ nhà không phản hồi và phí phát sinh." navigate={navigate} /><TableSection title="Hộp thư khiếu nại" headers={["Mã", "Loại", "SLA", "Trạng thái"]} rows={data.rows.disputes} /><ActionPanel title="DSP-102 - Hoàn tiền cọc" body="Khách báo chủ nhà hủy lịch sau khi đã nhận cọc giữ chỗ. Cần kiểm tra biên nhận và tin nhắn." action="Hoàn tiền" /></>; }
function Audit({ navigate, data }: AdminPageProps) { return <><AdminHeader title="Audit logs" body="Theo dõi hành động nhạy cảm: duyệt KYC, khóa tin, hoàn tiền, export dữ liệu và thay đổi phân quyền." navigate={navigate} /><TableSection title="Nhật ký hệ thống" headers={["Thời gian", "Người thao tác", "Hành động", "Kết quả"]} rows={data.auditRows} /><ActionPanel title="Xuất audit" body="Chỉ super admin được export log nhạy cảm theo khoảng thời gian và role." action="Xuất CSV" /></>; }
